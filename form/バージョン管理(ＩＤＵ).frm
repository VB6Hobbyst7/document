...
Begin VB.Form frmIDUVer 
   BorderStyle     =   0  'なし
   Caption         =   "                                                              ＩＤ中継ユニットバージョン管理"
   ClientHeight    =   9000
   ClientLeft      =   0
   ClientTop       =   -105
   ClientWidth     =   12000
...
'//                 EG20フェーズ２対応【結合TR-No.119】【結合TR-No.130】
...
'//                 北陸新幹線フェーズ３対応【HKRK_kansi03_001_01】
'//  備考：
'///////////////////////////////////////////////////////////////////
Option Explicit
...
Private Const HEADERTITLE_WRK = "IDUアプリケーションバージョン（ワーク）："
...
' EG20 V3.3.0.1【結合TR-No.119】 追加開始
...
' EG20 V3.3.0.1【結合TR-No.119】 追加終了
...
Private Sub sFDInstall()
    ...
    '「ワークコピー確認」ポップアップ画面表示
    iResponse = MsgBox("選択されたインストール部材の内容をＩＤＵアプリケーションの" _
                       & Chr(vbKeyReturn) _
...
' EG20 V30.4.0.1【HKRK_kansi03_001_01】 ADD Start
    ChDrive "C"
' EG20 V30.4.0.1【HKRK_kansi03_001_01】 ADD End
...
' EG20 V5.8.0.1追加開始
    ' 運改状態更新
    Call pubFuncUpdateUnkaiStatus(BootInfoHoshuType.TYPE_IDU, BOOTINFO_UNKAI_ARI)
    Call pubFuncAplUpdateUnkaiStatus(BOOTINFO_KEYNAMEIDU, BOOTINFO_UNKAI_ARI)
' EG20 V5.8.0.1追加終了
' EG20 V6.9.0.1【量産対応：アプリ切替改善対応】ADD START
    ' 切替実行コピーツールパラメータ更新処理
    Call funcUpdateCopyExecParam(KanendReq_ProcType.ML_DT_CLEAR_IDU)
' EG20 V6.9.0.1【量産対応：アプリ切替改善対応】ADD END

' EG20 V5.8.0.1 ADD START
    '読み取り外しの関数を実行
    dllChangeAttributeContents (PATH_IDU_APLNEW)
...
Private Sub cmdCopyOld_Jikko_Click()
    ...
    iResponse = MsgBox("「旧」フォルダの内容を、" _
            ...
            & Chr(vbKeyReturn) & "ＩＤＵの一世代前バージョンを､実行バージョンとします｡" _
...
'EG20 V3.6.0.1【03統合TR-No.372修正対応】追加開始
    ...
    ' 旧バージョン・EW4000JR・IDU・
    bRet = dllCheckAplVersion(4, PATH_IDU_APP, 0)
    If bRet = False Then
...
' EG20 V6.9.0.1【量産対応：アプリ切替改善対応】ADD START
    ' 切替実行コピーツールパラメータ更新処理
    bRet = funcUpdateCopyExecParam(KanendReq_ProcType.ML_DT_VERSIONDOWN_IDU)
    If bRet = False Then
        MsgBox "異常終了しました。", vbCritical, "旧→実行　コピー"
        Exit Sub
    End If
...
' EG20 V3.3.0.1【結合TR-No.119】削除開始
'' EG20 V2.1.0.1[Mainte_03_01]追加開始
'    ' アプリケーションバージョン切替実行処理
'    If (AplVersionChangeProc(ML_DT_VERSIONDOWN_IDU) = False) Then
'        ' // 保守を終了する。
'        Call psEndHoshuProc
'        '保守プロセス終了
'        End
'    End If
'' EG20 V2.1.0.1[Mainte_03_01]追加終了
' EG20 V3.3.0.1【結合TR-No.119】削除終了
' EG20 V3.3.0.1【結合TR-No.119】追加開始
...
' EG20 V5.2.0.1【結合TR-No.58修正対応】追加終了
    ...
    Else
        ...
        lngChangeKind = ML_DT_VERSIONDOWN_IDU       ' 切替種別を設定
    End If
' EG20 V3.3.0.1【結合TR-No.119】追加終了
...
Private Sub cmdCopyWork_Jikko_Click()
    ...
    iResponse = MsgBox("「ワーク」フォルダの内容を、" _
            ...
            & Chr(vbKeyReturn) & " ＩＤＵの最新バージョンを、実行バージョンとします。" _
            & Chr(vbKeyReturn) & "よろしいですか？", _
           vbOKCancel + vbExclamation, _
           "ワーク→実行 コピー")
    If iResponse = vbCancel Then
...
'EG20 V3.6.0.1【03統合TR-No.372修正対応】追加開始
    ...
    bRet = dllCheckAplVersion(1, PATH_IDU_APP, 0)
    If bRet = False Then
...
' EG20 V6.9.0.1【量産対応：アプリ切替改善対応】ADD START
    ' 切替実行コピーツールパラメータ更新処理
    bRet = funcUpdateCopyExecParam(KanendReq_ProcType.ML_DT_VERSIONUP_IDU)
    If bRet = False Then
        MsgBox "異常終了しました。", vbCritical, "ワーク→実行 コピー"
        Exit Sub
    End If
...
'' EG20 V2.1.0.1[Mainte_03_01]追加開始
'    ' アプリケーションバージョン切替実行処理
'    If (AplVersionChangeProc(ML_DT_VERSIONUP_IDU) = False) Then
'        ' // 保守を終了する。
'        Call psEndHoshuProc
'        '保守プロセス終了
'        End
...
' EG20 V5.2.0.1【結合TR-No.58修正対応】追加終了
    ...
    Else
        ...
        lngChangeKind = ML_DT_VERSIONUP_IDU         ' 切替種別を設定
    End If
' EG20 V3.3.0.1【結合TR-No.119】追加終了
...
Private Sub Form_Load()
   ...
   '「ID中継ユニットバージョン管理画面：表示」ログ出力
   Call sLogTraceReq(LTYP_NORMAL, L3AN_ETC, IDU_VERASION_KANRI_GAMEN_START, 0)
    
   gStrCurrentForm = sFormName_IDUVer
...
Private Sub cmdRefresh_Click()
   ...
   '「ID中継ユニットバージョン管理画面：表示更新釦押下」ログ出力
...
'*******************************
   ...
   '「ID中継ユニットバージョン管理画面：バージョン情報媒体出力釦押下」ログ出力
...
' EG20 V2.1.0.1[Mainte_03_01]追加開始
    ...
    '///////////////////////////////////////////////////////////////////////////////////////////
    bRet = dllCreateIDU_LDUVerFile(mlngChkFolderType, IDUVERLIST_REPORTFILE, PATH_IDU_APP, _
                                    VERLISTKIND_REPORT, 0)
...
' EG20 V5.13.0.1【プログレスバー表示機能見直し対応】追加終了
    ...
    'ファイルの有無確認
    If fso.FileExists(IDUVERLIST_REPORTFILE) = False Then
' EG20 V5.13.0.1【プログレスバー表示機能見直し対応】追加開始
        'プログレスバーを消去する
        Call SendMessageProgress(ML_ID_PRGBAR_HIDE_REQ)
' EG20 V5.13.0.1【プログレスバー表示機能見直し対応】追加終了
        ...
        Exit Sub
    End If
    strFileName = Dir(IDUVERLIST_REPORTFILE)
' EG20 V2.1.0.1[Mainte_03_01]追加終了
    
' EG20 V2.1.0.1[Mainte_03_01]削除開始
'    'ID中継ユニットバージョンファイル
'    strVerFile = PATH_IDU_APP & PATH_IDU_WORK & IDU_VER_FILE
'
'    'ファイルの有無確認
...
'
'    '作業エリアをNULLで初期化
'    strWork = String$(Len(strWork), vbNullChar)
'
'    '駅属性ファイルより設置駅名読み込み
'    iRet = GetPrivateProfileString(IDU_PROFILE_SECTION_NAME_STATION, _
'                                   IDU_PROFILE_KEY_NAME_EKIMEI, _
'                                   DEFAILT, strWork, Len(strWork), _
'                                   IDU_PROFILE_NAME)
'    '設置駅名取得失敗
'    If iRet = 0 Then
'        '「ID中継ユニットバージョン管理画面：バージョン情報媒体出力処理異常」ログ出力
'        lngErrCode = EDAI_KANSHI + ECHU_HOSHU + ESHO_FILE_IO + ECOD_AFILE
...
'    Else
'        '設置駅名取得
'        strEkimei = Left$(strWork, (InStr(strWork, vbNullChar) - 1))
'
'    End If
'
'    'コピー先フォルダパス作成(指定フォルダ￥設置駅名)
'    strWriteDir = strWriteDir & "\" & strEkimei
'
'    'コピー先フォルダの有無確認
'    If fso.FolderExists(strWriteDir) = False Then
'
...
' EG20 V2.1.0.1[Mainte_03_01]削除終了
    ...
    'コピー先ファイル名作成
'    strCopySaki = strWriteDir & "\" & IDU_VER_OUTPUTFILE       ' EG20 V2.1.0.1[Mainte_03_01]削除
' EG20 V2.1.0.1[Mainte_03_01]追加開始
    'コピー先ファイル名作成
    strCopySaki = strWriteDir & "\" & strStationName & "_" & strFileName

    'ファイルコピー（既に存在した場合は上書きするする）
    fso.CopyFile IDUVERLIST_REPORTFILE, strCopySaki, True
...
' EG20 V5.13.0.1【プログレスバー表示機能見直し対応】追加終了
    ...
    '「ID中継ユニットバージョン管理画面：バージョン情報媒体出力処理正常」ログ出力
    Call sLogTraceReq(LTYP_NORMAL, L3AN_FILE, VERASION_INFO_OUTPUT_OK, 0)
    
    Set fso = Nothing   ' EG20 V2.1.0.1[Mainte_03_01]追加
    Exit Sub
'*******************************
'VBエラー処理
...
Private Sub cmdCancel_Click()
   ...
   '「ID中継ユニットバージョン管理画面：消去」ログ出力
   Call sLogTraceReq(LTYP_NORMAL, L3AN_ETC, IDU_VERASION_KANRI_GAMEN_END, 0)
   frmVersion.ZOrder
   Unload Me
End Sub
...
' EG20 V2.1.0.1[Mainte_03_01]追加終了
    ...
    'ID中継ユニット画面表示用バージョンファイルパス作成
    strFilePath = PATH_IDU_APP & PATH_IDU_WORK & IDU_VER_FILE
    bRet = True
    '///////////////////////////////////////////////////////////////////////////////////////////
    '/ 保守専用関数:ID中継ユニット画面表示用バージョンファイル作成
    '///////////////////////////////////////////////////////////////////////////////////////////
'    bRet = dllCreateIDU_LDUVerFile(mlngChkFolderType, strFilePath, PATH_IDU_APP)                       ' EG20 V2.1.0.1[Mainte_03_01]削除
    bRet = dllCreateIDU_LDUVerFile(mlngChkFolderType, strFilePath, PATH_IDU_APP, VERLISTKIND_DISP, 0)   ' EG20 V2.1.0.1[Mainte_03_01]追加

    'ID中継ユニット画面表示用バージョンファイル成功
    If bRet Then
       '「ID中継ユニットバージョン管理画面：バージョン情報ファイル作成正常」ログ出力
       Call sLogTraceReq(LTYP_NORMAL, L3AN_FILE, CREATE_FILE_OK, 0)
    'ID中継ユニット画面表示用バージョンファイル失敗
    Else
       '「ID中継ユニットバージョン管理画面：バージョン情報ファイル作成異常」ログ出力
       lngErrCode = EDAI_KANSHI + ECHU_HOSHU + ESHO_FILE_IO + ECOD_AFILE
       Call sLogTraceReq(LTYP_ERROR, L3AN_FILE, CREATE_FILE_ERROR, lngErrCode)
       Exit Sub
    End If
    
    'ID中継ユニット画面表示用バージョンファイルの有無確認
    If Len(Trim(Dir(strFilePath))) = 0 Then
        Exit Sub
    End If

    'ID中継ユニット画面表示用バージョンファイルのファイル番号を取得する。
    intFileNo = FreeFile

    'ID中継ユニット画面表示用バージョンファイルオープン
...
'    If pfComMailRecieve = ML_ID_HOSHU_ACTIVE_REQ Then          ' EG20 V3.0.0.2削除
    If pfVersionDispMailRecieve = ML_ID_HOSHU_ACTIVE_REQ Then   ' EG20 V3.0.0.2追加
       '保守画面アクティブ要求を受信したら、自画面を前面に表示させる。
        AppActivate frmIDUVer.Caption, False
        pfFormActive (frmIDUVer.hwnd)
    End If
End Sub
'V1.3.0.1 ADD END
