VERSION 5.00
Begin VB.Form frmICUnkai_Type2 
   BorderStyle     =   0  'なし
   Caption         =   "運賃データＤＬＬ画面"
   ClientHeight    =   9000
   ClientLeft      =   1380
   ...
   WindowState     =   2  '最大化
   Begin VB.Frame fraICUntin 
      Height          =   1215
      Left            =   9600
      TabIndex        =   8
      Top             =   6000
      Width           =   2055
      Begin VB.OptionButton optICUntin 
         Caption         =   "現運賃"
         BeginProperty Font 
            Name            =   "ＭＳ ゴシック"
            Size            =   12
      ...
      End
   End
   Begin VB.Frame fraJikiUntin 
      Height          =   1215
      Left            =   9600
      TabIndex        =   5
      Top             =   1800
      Width           =   2055
      Begin VB.OptionButton optJikiUntin 
         Caption         =   "新運賃"
         BeginProperty Font 
            Name            =   "ＭＳ ゴシック"
            Size            =   12
   ...
   End
End
Attribute VB_Name = "frmICUnkai_Type2"
...
'*      REVISIONS :(MM2.0.0.1) 2008-08-21   REVISED BY [TCC] C.Terui
'*               舞浜：ＩＣ共通運改対応（メトロをベースに作成）
...
Private Sub cmdVer_Click(Index As Integer)
    ...
    Dim iDataSts As Integer             'V2.2.0.1 ADD
    
    Dim iIndex As Integer 'ST V3.0.0.1 ADD
...
'MM V2.0.0.1 ADD START
    '現運賃／新運賃選択ラジオボタンを押下不可にする。
    fraJikiUntin.Enabled = False
    For iIndex = 0 To optJikiUntin.UBound
        optJikiUntin(iIndex).Enabled = False
    Next
    fraICUntin.Enabled = False
    For iIndex = 0 To optICUntin.UBound
        optICUntin(iIndex).Enabled = False
    Next
'MM V2.0.0.1 ADD END
...
''        gSendMailShosai = MlUnchinDllSHUBETSU.ML_DT_UNCHIN_NEW   '送信メール：詳細             'MM V2.0.0.1 DEL
'        'データ種別
'        If gIndex = DLL_DATA.JIKI Then      '磁気運賃ＤＬＬ
''            gSendMailShubetsu = MlUnchinDllData.ML_DT_UNCHIN_ICHI_FUKU    '１枚用＋複数枚用    'MM V2.0.0.1 DEL
'            gSendMailShubetsu = MlUnchinDllData.ML_DT_UNCHIN_ICHIMAI       '運賃               'MM V2.0.0.1 ADD
''MM V2.0.0.1 ADD START
'            '詳細種別設定
'            Select Case True
'                '現運賃
'                Case optJikiUntin(0)
'                    gSendMailShosai = MlUnchinDllSHUBETSU.ML_DT_UNCHIN_NOW
'                '新運賃
'                Case optJikiUntin(1)
'                    gSendMailShosai = MlUnchinDllSHUBETSU.ML_DT_UNCHIN_NEW
'            End Select
''MM V2.0.0.1 ADD END
'        Else                                'ＩＣ運賃ＤＬＬ
...
'        udtMail.dwSyosai = gSendMailShosai              '種別詳細：現運賃 or 新運賃
'
...
Private Sub tmrMail_Timer()
    ...
    Dim iIndex As Integer                   'MM V2.0.0.1 ADD
    'メール受信
...
'MM V2.0.0.1 ADD START
            '現運賃／新運賃選択ラジオボタンを押下可能にする。
            fraJikiUntin.Enabled = True
            For iIndex = 0 To optJikiUntin.UBound
                optJikiUntin(iIndex).Enabled = True
            Next
            fraICUntin.Enabled = True
            For iIndex = 0 To optICUntin.UBound
                optICUntin(iIndex).Enabled = True
            Next
'MM V2.0.0.1 ADD END
            ...
            AppActivate frmICUnkai_Type2.Caption, False 'V2.2.0.1 ADD
