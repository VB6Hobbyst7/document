VERSION 5.00
Begin VB.Form frmInstallformat 
   BorderStyle     =   0  'なし
   ...
   End
   Begin VB.Label lblHelp 
      Caption         =   "・Ｉ Ｄ Ｕ  ：DBデータ、バックアップデータ、再送データ、その他データ"
      BeginProperty Font 
         Name            =   "ＭＳ Ｐゴシック"
         Size            =   9.75
         Charset         =   128
   ...
   End
   Begin VB.Label lblHelp 
      Caption         =   "・統合監視盤 ：集計関連データ、バックアップデータ"
      BeginProperty Font 
         Name            =   "ＭＳ Ｐゴシック"
         Size            =   9.75
         Charset         =   128
   ...
   End
End
Attribute VB_Name = "frmInstallformat"
...
'//
'//  ファイル名  ：frmInstallformat.frm
'//  パッケージ名：設置時初期化画面
'/
'//  概要：設置時初期化画面
'//     ORIGINAL  :(EG20 V6.9.0.1) 2012-12-05  CODED BY  [TCC] H.Sugimoto
'//                 量産対応【設置時初期化機能追加】
'//
'//  備考：一括初期化画面.frm（frmALLSysformat.frm）を流用
'///////////////////////////////////////////////////////////////////
Option Explicit
...
' EG20 V6.9.0.1 【量産対応：設置時初期化機能追加】DEL START
'    '「一括システム初期化：表示」ログ出力
'    Call sLogTraceReq(LTYP_NORMAL, L3AN_ETC, ALL_SYSFORMAT_GAMEN_START, 0)
' EG20 V6.9.0.1 【量産対応：設置時初期化機能追加】DEL END
' EG20 V6.9.0.1 【量産対応：設置時初期化機能追加】ADD START
    '「設置時初期化：表示」ログ出力
    Call sLogTraceReq(LTYP_NORMAL, L3AN_ETC, INSTALL_FORMAT_GAMEN_START, 0)
...
' EG20 V6.9.0.1 【量産対応：設置時初期化機能追加】ADD START
    '「設置時初期化画面：消去」ログ出力
    Call sLogTraceReq(LTYP_NORMAL, L3AN_ETC, INSTALL_FORMAT_GAMEN_END, 0)
    frmInstallformat.ZOrder
' EG20 V6.9.0.1 【量産対応：設置時初期化機能追加】ADD END
    Unload Me
End Sub
...
' EG20 V6.9.0.1 【量産対応：設置時初期化機能追加】DEL END
' EG20 V6.9.0.1 【量産対応：設置時初期化機能追加】ADD START
        AppActivate frmInstallformat.Caption, False
        pfFormActive (frmInstallformat.hwnd)
' EG20 V6.9.0.1 【量産対応：設置時初期化機能追加】ADD END
    End If
End Sub
'V1.3.0.1 ADD END
