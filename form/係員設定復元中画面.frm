...
Private Sub Form_Load()
    ...
    'プログレスバーを表示する
    Call SendMessageProgress(ML_ID_PRGBAR_SHOW_REQ, PRG_KAKARI_RENEW)
    ...
    If lngLength > 0 Then
        ...
        Select Case udtReadMail.udtlHeader.dwId        'メールＩＤ
            ...
            Case ML_ID_HOSHU_ACTIVE_REQ
                ...
                AppActivate frmRenewCyu.Caption, False
                pfFormActive (frmRenewCyu.hwnd)     ' EG20 V8.1.0.1【EG20_KANSI05_01】ADD
            Case ML_ID_KAKARIIN_RENEW_RES
                '「係員復元完了通知」を受信した場合
                '「係員復元完了通知受信正常」ログ出力
                Call sLogTraceReq(LTYP_NORMAL, L3AN_RECV, KAKARIIN_RENEW_REQ_RECV, 0)
                'クリア通知内容をチェックする。
...
Private Function fSDATAMailSend() As Boolean
    ...
    '係員設定復元要求を送信する
    udtMail.mlHeader.dwId = ML_ID_KAKARIIN_RENEW_CMD
    ...
    If bRet = False Then
       '「係員設定復元復元画面：係員設定復元要求送信異常」ログ出力
       lngErrCode = EDAI_KANSHI + ECHU_HOSHU + ESHO_MAIL_IO + ECOD_MSEND
       Call sLogTraceReq(LTYP_ERROR, L3AN_SEND, KAKARIIN_RENEW_REQ_SEND, lngErrCode)
        'プログレスバーを消去する
        Call SendMessageProgress(ML_ID_PRGBAR_HIDE_REQ)
