...
'    クリア中のガイドを表示する｡
'EG20 V2.0.1.1【監視D-159】 DEL START
'    lblMessage(0) = "時間帯別データクリア中です。"
'    lblMessage(1) = "しばらくお待ちください。"
'EG20 V2.0.1.1【監視D-159】 DEL END
'EG20 V2.0.1.1【監視D-159】 ADD START
    lblMessage(0) = "処理中です。"
    lblMessage(1) = ""
'EG20 V2.0.1.1【監視D-159】 ADD END
    cmdOK.Enabled = False
    ...
    '受信メールがあれば、メールＩＤ毎の処理をする。
        Select Case udtReadMail.udtlHeader.dwId        'メールＩＤ
            ...
            Case ML_ID_HOSHU_ACTIVE_REQ
                ...
                pfFormActive (frmClearCyu.hwnd)         ' EG20 V8.1.0.1【EG20_KANSI05_01】ADD
            Case ML_ID_TIMEDATA_CLEAR_RES
                '「時間帯別データクリア要求RES」を受信した場合
                '「時間帯別データクリア受信正常」ログ出力
...
Private Function fCDATAMailSend() As Boolean
    ...
    Dim udtMail As MAIL_INFO_CMD    '時間帯別データクリア指示メール送信エリア
...
' EG20 V5.13.0.1【プログレスバー表示機能見直し対応】追加終了
       ...
       Exit Function
    Else
       '「時間帯別データ設定画面：時間帯別データクリア指示送信正常」ログ出力
